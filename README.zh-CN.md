...
- 只读 GitHub Pull Request Review Boundary
...
Context Lab、公开 GitHub PR Review、Resume provenance、使用 `deepseek-v4-flash` 的 Prompt Experiment，以及 Prompt Library 的持久化与故障隔离均完成过真实 GUI / REST 验证。

## 当前限制

- Research SSE 会在同步执行完成后 replay 可用事件，并非原生 Token / Tool Streaming。
- PKRA 当前 Workbench Contract 尚未暴露无损结构化 Sources / Evidence 与 Activity Trace。
- Context Token 数量为确定性估算，不是 Provider Tokenizer 的精确计数。
- Prompt Experiment 每次只运行一个所选 Variant，没有自动 Comparison Matrix。
- Prompt Library 当前没有 Authentication、Multi-user Workflow、Pagination 或 Version History。
